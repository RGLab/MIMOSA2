...
```{r}
fit = MIMOSA2(Ntot=s$Ntot,ns1=s$ns1,nu1=s$nu1,ns0=s$ns0,nu0=s$nu0,tol=1e-10)
```
...
We have `sum(table(fit=colnames(fit$inds)[max.col(fit$inds)],truth = s$truth)["R",-4L])` false positives, and `sum(table(fit=colnames(fit$inds)[max.col(fit$inds)],truth = s$truth)[,4][-3])` false negatives.
...
set.seed(100)
r = replicate(50,simulate_MIMOSA2())

fits = foreach(i=seq_along(r), .errorhandling = "remove") %dorng% 
{
  x=r[,i]
  MIMOSA2fit(
    Ntot = x$Ntot,
...
R = foreach (i = seq_along(fits), .errorhandling="remove",.combine="rbind") %dorng% {
  x = r[, i]
  or = ORTest(
  Ntot = x$Ntot,
  ns1 = x$ns1 ,
