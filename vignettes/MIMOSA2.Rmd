...
```{r sim}
set.seed(100)
s = simulate_MIMOSA2(effect=5e-5, baseline_stim_effect=5e-5,baseline_background = 2e-5,bg_effect =2e-5,phi=c(50000,50000,50000,50000),P=100,rng=c(120000,150000))
```
...
```{r}
foo = function(fit,truth){
  th = MIMOSA:::fdr(cbind(1-rowSums(fit$z[,1:4]),fit$z[,1:4]))
  o = order(th,decreasing=FALSE)
  tpc = cumsum(truth[o])
  fpc = cumsum(!truth[o])
  plot(y=tpc,th[o],type="l",ylab="Count",xlab="FDR Threshold",col="blue")
  lines(y=fpc,th[o],type="l",col="red")
  legend("top",c("TPC","FPC"),col=c("blue","red"),lty=1,cex=0.75)
}
foo2 = function(thresh,truth,add=FALSE,lty=3,...){
  th = thresh
  o = order(th,decreasing=FALSE)
  tpc = cumsum(truth[o])
  fpc = cumsum(!truth[o])
...
foo2(MIMOSA::fdr(cbind(1-rowSums(fit$z[,1:4]),rowSums(fit$z[,1:4]))),s$truth%in%c("R1","R2","R3","R4"))
foo2(ortest,s$truth%in%c("R1","R2","R3","R4"),add=TRUE)
legend("topleft",lty=c(1,2),c("MIMOSA","OR Test"))
```
...
plot(y=fdr[o],x=cumsum(!s$truth[o]%in%c("R1","R2","R3","R4"))/1:length(s$truth),type="l",ylab="observed fdr",xlab="nominal fdr",xlim=c(0,1),ylim=c(0,1),col="blue")
abline(0,1,lty=2)
...
```{r}
ggplot(data.frame(deltap=with(fit,ps1_hat-pu1_hat-ps0_hat+pu0_hat),response=factor(s$truth%in%c("R1","R2","R3","R4"),labels=c("Non Responder","Responder"))))+geom_boxplot(outlier.col=NA)+geom_jitter()+aes(x=response,y=deltap)+theme_bw()+scale_y_continuous("ps1-pu1-(ps0-pu0)")
ggplot(data.frame(deltap=with(fit,ps1_hat-pu1_hat),response=factor(s$truth%in%c("R1","R2","R3","R4"),labels=c("Non Responder","Responder"))))+geom_boxplot(outlier.col=NA)+geom_jitter()+aes(x=response,y=deltap)+theme_bw()+scale_y_continuous("ps1-pu1")
ggplot(data.frame(deltap=with(fit,ps0_hat-pu0_hat),response=factor(s$truth%in%c("R1","R2","R3","R4"),labels=c("Non Responder","Responder"))))+geom_boxplot(outlier.col=NA)+geom_jitter()+aes(x=response,y=deltap)+theme_bw()+scale_y_continuous("ps0-pu0")
```
