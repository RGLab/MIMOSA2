...
Under models 1-4, we have responses, with the following constraints:  
$p_i^{(s,1)} - p_i^{(u,1)} > p_i^{(s,0)} - p_i^{(u,0)}$ and $p_i^{(s,1)} > p_i^{(u,1)}$.  
...
```{r sim}
set.seed(100)
s = simulate_MIMOSA2(effect=5e-4, baseline_stim_effect=2.5e-4,phi=1000,P=100)
```
...
```{r}
load_all()
fit = with(s, MIMOSA2(Ntot=Ntot,ns1=ns1,nu1=nu1,ns0=ns0,nu0=nu0,tol=1e-10))
...
```{r confusion}
table(fit=c("R1","R2","R3","R4","NR1","NR2","NR3","NSR")[max.col(fit$inds)],truth = s$truth)
```
...
ggplot(melt(p,value.var=c("pu1","ps1","pu0","ps0")))+geom_boxplot(aes(x=truth,y=value))+facet_wrap(~variable)
ggplot(p)+geom_boxplot(aes(x=truth,y=ps1-pu1-ps0+pu0))
```
...
plot(fdr[o],cumsum(!s$truth[o]%in%c("R1","R2","R3","R4"))/1:length(s$truth),type="l",ylab="true fdr",xlab="nominal fdr",xlim=c(0,0.25),ylim=c(0,0.25))
abline(0,1,lty=2)
o = order(ortest,decreasing=FALSE)
lines(ortest[o],cumsum(!s$truth[o]%in%c("R1","R2","R3","R4"))/1:100,type="l",ylab="true fdr",xlab="nominal fdr",col="red")
abline(0,1,lty=2)
```
